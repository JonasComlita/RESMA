...
const STOP_CAPTURE_RECOMMENDATION_WAIT_MS = 1500;
const STOP_CAPTURE_RECOMMENDATION_POLL_MS = 250;
...
class YouTubeObserver {
    ...
    private startVideoTracking(videoId: string, captureSurface: CaptureSurface) {
        ...
        const entry: YouTubeVideo = {
            ...
            channelHandle: this.normalizeChannelHandle(
                document.querySelector('ytd-video-owner-renderer a[href^="/@"]')?.getAttribute('href') || null
            ),
    ...
    private getRecommendationCountForVideo(videoId: string): number {
        const entry = this.getActiveEntry();
        if (!entry || entry.videoId !== videoId) {
            return 0;
        }
        return entry.recommendations.length;
    }

    private async waitForRecommendationsBeforeStop() {
        const videoId = this.activeVideoId;
        if (!videoId) {
            return;
        }

        this.scrapeRecommendations(videoId);
        if (this.getRecommendationCountForVideo(videoId) > 0) {
            return;
        }

        const deadline = Date.now() + STOP_CAPTURE_RECOMMENDATION_WAIT_MS;
        while (Date.now() < deadline && this.activeVideoId === videoId) {
            await new Promise((resolve) => {
                window.setTimeout(resolve, STOP_CAPTURE_RECOMMENDATION_POLL_MS);
            });
